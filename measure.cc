...
static obj_t sample(pset_t pset) {
  ...
  string cmd = size ?
    string("size -A ") + tmp_file.path + " | grep .text | awk '{ print $2 }'" :
    tmp_file.path;
...
// all results
typedef map<pset_t, obj_t> results_t;
results_t results;
...
point_t get_min_point() {
  ...
  results_t::const_iterator min_i = results.begin();
  for (results_t::const_iterator i = results.begin(); i != results.end(); ++i)
